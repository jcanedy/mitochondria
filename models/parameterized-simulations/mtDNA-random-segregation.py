...
for mtDNATotal in mtDNATotals:

	# Mitochondria per cell
	initA = mtDNATotal
	initAlpha = mtDNATotal
	...
	while True:
		...
		percent = percentHomoplasmic(cells)
	 	...
	 	sys.stderr.write('{3} | Generation: {0} | Homoplasmic: {1:.2%} | Mean Cell Size {2:.2f}\r'.format(generation, percent, np.mean(cellSize(cells)), 2*mtDNATotal))
		sys.stderr.flush()
	...
	print "%.2f,%d" % (mtDNATotal*2, generation)

mpl_fig = plt.figure()

plt.scatter(buddingProportions, generationsToHomoplasmy)
plt.xlabel('mtDNA Total')
