...
title = "Random Growth: Simulation " + title
...
initA = 25.0
initAlpha = 25.0
mtDNAPerCell = initA + initAlpha

# Helper Functions

# grow(n, p)
def grow(cell):
	cell[A] = np.random.binomial(mtDNAPerCell, float(cell[A]) / float(cell[A] + cell[ALPHA]))
	cell[ALPHA] = np.random.binomial(mtDNAPerCell, float(cell[ALPHA]) / float(cell[A] + cell[ALPHA]))
	return cell
